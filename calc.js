...
export function session(group, i, type, offsets) {
  const r = type.mode === 'laps' ? laps(group, i, type)
          : type.mode === 'table' ? table(i, type)
          : sets(group, i, type, offsets);
  return {
    ...r,
    repPace: pace(r.rep),
    recPace: pace(r.recIsTime ? r.rec * 400 / type.rec : r.rec),
    backPace: r.back ? pace(r.back) : null,
    repTime: clock(r.rep * type.rep / 400),
    // 표형은 회복이 통째로 초 단위, 나머지는 랩타임 x 거리
    recTime: clock(r.recIsTime ? r.rec : r.rec * type.rec / 400),
